...
const _rules: Record<RuleId, Rule> = {
  ...
  noSpacedHyphen: {
    validator: value => !/\s-\s/.test(value),
  ...
  onlyLatinLetters: {
    validator: value => /^[a-z0-9-!:&.+'/»#%°$@?|¡–\s_—]+$/i.test(value),
    message: label =>
  ...
  dateBeforeToday: {
    validator: value => {
      ...
      return dayjs(value, 'YYYY-MM-DD').isBefore(today)
    },
  ...
  validWikidataId: {
    validator: value => /^Q\d+$/.test(value),
...
export function validate(label: string, value: any, rules: RuleId[]) {
  const errors: string[] = []
